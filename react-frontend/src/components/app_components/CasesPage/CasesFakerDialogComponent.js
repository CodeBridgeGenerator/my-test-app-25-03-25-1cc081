...
import casesFakerFactory from "./casesFakerFactory";
...
const CasesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = casesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cases").create(elem));
...
export default connect(mapState, mapDispatch)(CasesFakerDialogComponent);
