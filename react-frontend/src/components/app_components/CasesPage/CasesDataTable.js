...
const CasesDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
<Column field="caseNo" header="Case No" body={pTemplate1} filter={selectedFilterFields.includes("caseNo")} hidden={selectedHideFields?.includes("caseNo")}  sortable style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload Cases Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="cases"            
          onUploadComplete={() => {
      ...
      <Dialog header="Search Cases" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
